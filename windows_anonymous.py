...
class Gui:
    def __init__(self, master):
        ...
        self.label = Label(self.frame_name, text="Your name must be from 2 to 10 characters long.", font=("Arial", 11))
        self.label.pack()

        self.label_name = Label(self.frame_name, text="Enter your name: ", font=("Arial", 11))
        self.label_name.pack(side=LEFT)

        self.entry = Entry(self.frame_name, width=23)
        self.entry.bind("<Return>", self.check_value_name_press)
        self.entry.pack(side=LEFT)

        self.button = Button(self.frame_name, text="Enter", width=8, command=self.check_value_name_click)
        ...
        self.messages = Text(self.frame, width=46, height=23)
        ...
        self.ip_label = Label(self.frame_connect, text="Ip address: ", font=("Arial", 11))
        self.ip_label.pack(side=LEFT)

        self.original_color_text = self.ip_label.cget("foreground")

        self.ip_entry = Entry(self.frame_connect, width=21)
        ...
        self.port_label = Label(self.frame_connect, text="Port: ", font=("Arial", 11))
        self.port_label.pack(side=LEFT)

        self.port_entry = Entry(self.frame_connect, width=8)
        ...
        self.button_connect = Button(self.frame_connect, text="Connect", width=8, command=self.socket_connect_thread)
        self.button_connect.bind('<Enter>', lambda _: self.tip_button_connect.place(x=153, y=350))
        ...
        self.ip_label2 = Label(self.frame_bind, text="Ip address: ", font=("Arial", 11))
        self.ip_label2.pack(side=LEFT)

        self.ip_entry2 = Entry(self.frame_bind, width=21)
        ...
        self.port_label2 = Label(self.frame_bind, text="Port: ", font=("Arial", 11))
        self.port_label2.pack(side=LEFT)

        self.port_entry2 = Entry(self.frame_bind, width=8)
        ...
        self.button_bind = Button(self.frame_bind, text="Bind", width=8, command=self.socket_binding_thread)
        self.button_bind.bind('<Enter>', lambda _: self.tip_button_bind.place(x=204, y=350))
        ...
        self.button_send = Button(self.frame_send, text="Send", width=11, command=self.sending_messages_click)
        self.button_send.pack(side=LEFT, fill=Y)

        self.text_send = Text(self.frame_send, width=35, height=5)
        ...
        self.tip_button_connect = Label(self.frame, text='Start trying to connecting to your friend.', background="white")
        ...
        self.tip_button_bind = Label(self.frame, text='Start listening for connections.', background="white")
    ...
    def sending_messages_click(self):
        ...
        if len(message) > 0 and not message.isspace():
            ...
            if "->clear()" in self.message:
                ...
                self.port_entry.configure(background=self.original_color_background,
                                          foreground=self.original_color_text)
                self.button_connect.configure(background=self.original_color_background,
                                              foreground=self.original_color_text)
                self.ip_entry2.configure(background=self.original_color_background, foreground=self.original_color_text)
                self.port_entry2.configure(background=self.original_color_background,
                                           foreground=self.original_color_text)
                self.button_bind.configure(background=self.original_color_background,
                                           foreground=self.original_color_text)
                self.button_send.configure(background=self.original_color_background,
                                           foreground=self.original_color_text)
                ...
                self.port_label.configure(background=self.original_color_background,
                                          foreground=self.original_color_text)
                self.ip_label2.configure(background=self.original_color_background, foreground=self.original_color_text)
                self.port_label2.configure(background=self.original_color_background,
                                           foreground=self.original_color_text)
                ...
                self.tip_ip_connect.configure(background=self.original_color_background,
                                              foreground=self.original_color_text)
                self.tip_port_connect.configure(background=self.original_color_background,
                                                foreground=self.original_color_text)
                self.tip_button_connect.configure(background=self.original_color_background,
                                                  foreground=self.original_color_text)
                self.tip_ip_bind.configure(background=self.original_color_background,
                                           foreground=self.original_color_text)
                self.tip_port_bind.configure(background=self.original_color_background,
                                             foreground=self.original_color_text)
                self.tip_button_bind.configure(background=self.original_color_background,
                                               foreground=self.original_color_text)
            ...
            if len(str(ip)) <= 15 and len(str(ip)) >= 7 and len(str(port)) <= 5 and len(str(port)) >= 4:
                ...
                if str(ip) == str(ip_connect) and str(port) == str(port_connect):
                    ...
                    if threading.active_count() < 4:
                        ...
                        self.button_bind = Button(self.frame_bind, text="Unbind", width=8,
                                                  command=self.socket_binding_stop)
                        self.button_bind.pack(side=LEFT)
                        ...
                        while not self.quiting:
                            ...
                            if data:
                                ...
                                self.button_bind.pack_forget()
                                self.button_bind = Button(self.frame_bind, text="Bind", width=8,
        ...
        try:
            ...
            if len(str(ip)) <= 15 and len(str(ip)) >= 7 and len(str(port)) <= 5 and len(str(port)) >= 4:
                ...
                if str(ip) == str(ip_bind) and str(port) == str(port_bind):
                    ...
                    self.button_connect = Button(self.frame_connect, text="Disconnect", width=8,
